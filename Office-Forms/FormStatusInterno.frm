VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} FormStatusInterno 
   Caption         =   "Status Interno"
   ClientHeight    =   6000
   ClientLeft      =   120
   ClientTop       =   450
   ClientWidth     =   5235
   OleObjectBlob   =   "FormStatusInterno.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "FormStatusInterno"
...
'    If Right(lblTotalHoras, 2) <> "00" And Right(lblTotalHoras, 2) <> "30" Or Len(tbxDataInicial) < 10 Or Len(tbxDataFinal) < 10 Or Len(tbxHoraInicial) < 5 Or Len(tbxHoraFinal) < 5 Or (optSolicitacaoClienteSim.value = "Falso" And optSolicitacaoClienteNao.value = "Falso") Or (optEncerraAtividadeSim.value = "Falso" And optEncerraAtividadeNao.value = "Falso") Then
...
Private Sub optEncerraAtividadeNao_Click()
    If Right(lblTotalHoras, 2) <> "00" And Right(lblTotalHoras, 2) <> "30" Or Len(tbxDataInicial) < 10 Or Len(tbxDataFinal) < 10 Or Len(tbxHoraInicial) < 5 Or Len(tbxHoraFinal) < 5 Or (optEncerraAtividadeSim.value = "Falso" And optEncerraAtividadeNao.value = "Falso") Then
        If Right(lblTotalHoras, 2) <> "00" And Right(lblTotalHoras, 2) <> "30" Then
            lblTotalHoras.ForeColor = &HFF&
        Else
            lblTotalHoras.ForeColor = &HC000&
...
Private Sub optEncerraAtividadeSim_Click()
    If Right(lblTotalHoras, 2) <> "00" And Right(lblTotalHoras, 2) <> "30" Or Len(tbxDataInicial) < 10 Or Len(tbxDataFinal) < 10 Or Len(tbxHoraInicial) < 5 Or Len(tbxHoraFinal) < 5 Or (optEncerraAtividadeSim.value = "Falso" And optEncerraAtividadeNao.value = "Falso") Then
        If Right(lblTotalHoras, 2) <> "00" And Right(lblTotalHoras, 2) <> "30" Then
            lblTotalHoras.ForeColor = &HFF&
        Else
...
Private Sub UserForm_Activate()
    ...
    optEncerraAtividadeSim.value = False
    optEncerraAtividadeNao.value = False
...
Private Sub btnCancel_Click()
    ...
    Projeto1.FormStatusInterno.Hide
End Sub
...
Private Sub btnOK_Click()
    ...
    If Left(lblTotalHoras, 2) > 4 Then
        ...
        Else
            ...
            If (optEncerraAtividadeSim.value = "Verdadeiro" Or optEncerraAtividadeSim.Enabled = False) Then
                EncerramentoAtividade = "S"
            Else
                EncerramentoAtividade = "N"
            End If
